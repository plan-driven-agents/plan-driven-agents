...
The `.claude-plugin/` directory is generated from the `definitions/` modules at build time, not hand-maintained. The OpenCode plugin (`src/index.ts`) imports the same `definitions/` modules at runtime. Both targets share one source of truth. CI runs the generator on every PR and fails if the committed output differs from what the generator produces, which catches hand-edits to the generated files.
